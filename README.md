...
Masking is performed via the `sigprocmask()` function. When `preempt_save()` is called, the interrupt is masked to prevent the critical section from being interrupted. The mask value and current timer value are stored in the thread's `sigset_t->__val[0]`. It then calls `preempt_disable()` which uses the `setitimer()` function to set the value of the timer to fully disable it. The interrupt is not unmasked until after the timer has been re-enabled with the restored value which was extracted from the thread's sigset_t->__val[0] in `preempt_restore()`.
