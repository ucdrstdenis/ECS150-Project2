...
int queue_dequeue(queue_t queue, void **data)
{
	...
	if (data == queue->head->data)						/* Check if data equals to head data 	*/
		queue->head = (queue->head)->next;				/* Set head as next node in queue		*/
...
{
	node *curr = (node*) malloc(sizeof(node));			/* Allocate memory for node pointer		*/
	...
	while (curr != NULL)								/* Iterate through the queue			*/
		//Call function 'func'
		curr = curr->next;								/* Iterate to next node in queue		*/
