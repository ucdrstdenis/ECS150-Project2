...
{
	utcb *ready, *run;									/* Declare TCB pointers for ready & run		*/

	//Running
	queue_dequeue(RunQ, (void**) &run);
	uthread_enqueue(run, BLOCKED);

	//Ready
	queue_dequeue(ReadyQ, (void**) &ready);
	uthread_enqueue(ready, READY);
...
{
	queue_delete(DoneQ, (void*) uthread);
	uthread_enqueue(uthread, READY);
